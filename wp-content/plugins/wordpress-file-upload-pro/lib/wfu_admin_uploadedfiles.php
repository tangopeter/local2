...
function wfu_uploadedfiles_menu() {
		...
		/** This action is documented in lib/wfu_admin.php */
...
function wfu_uploadedfiles_manager($page = 1, $only_table_rows = false) {
	...
	foreach ( $filerecs as $filerec ) {
		...
		$remote = false;
		$service = wfu_check_file_remote($file_abspath);
		if ( $service !== false ) {
			...
			if ( $metadata != null ) {
				...
				$file_relpath = $service.":".$destination.$metadata['filename'];
			}
		}
		...
		$file_is_local = $file_is_local && !$remote;
		...
		//check if there are any filetransfer actions and store the associated
		//service
		$transfer_services = array();
		foreach ( $historyrecs as $rec ) {
			if ( $rec->action == "filetransfer" ) {
				$data = wfu_get_filedata_from_rec($rec, true);
				if ( isset($data['log']) ) {
					extract(wfu_read_log_data($data["log"]["data"]));
					array_push($transfer_services, $service);
				}
			}
		}
		$transfer_services_uploaded = array();
		//check if file has been transferred to a service
		foreach ( $filedata as $key => $data ) {
			if ( isset($data['type']) && $data['type'] == "transfer" ) {
				$status = $data['status'];
				$destination = ( $data['destination'] == '' ? 'root' : $data['destination'] );
				//correct bug where transferred files are shown in
				//"uploading" status in plugin versions before 4.6.3
				if ( $status == "uploading" && in_array($key, $transfer_services) ) $status = "uploaded";
				if ( $status == "uploaded" ) {
					$properties[$key]['visible'] = true;
					$properties[$key]['remarks'] = 'File has been transferred to <strong>'.wfu_service_get_name($key).'</strong>:'.$destination;
					array_push($transfer_services_uploaded, $key);
				}
				elseif ( $status == "uploading" ) {
					//check that upload is valid
					if ( $wpdb->get_var('SELECT COUNT(*) FROM '.$table_name3.' WHERE iddbxqueue = '.$data['id']) > 0 ) {
						$properties[$key]['visible'] = true;
						$properties[$key]['title'] = 'File is being transferred to '.wfu_service_get_name($key);
						$properties[$key]['remarks'] = 'File is being transferred to <strong>'.wfu_service_get_name($key).'</strong>:'.$destination;
					}
				}
			}
		}
		...
		*/
		...
		elseif ( $remote && !$obsolete && $actions['download']['allowed'] && $metadata['downloadLink'] != '' ) {
			$download_href = $metadata['downloadLink'];
			$actions['download']['visible'] = true;
			$actions['download']['href'] = $download_href;
			$actions['download']['newtab'] = false;
			$actions['download']['color'] = 'blueviolet';
		}
		//check if there are remote files successfully uploaded that have links
		//in order to set remote link actions
		foreach ( $transfer_services_uploaded as $service ) {
			$metadata = wfu_get_remote_file_metadata($filedata, $service);
			if ( !$obsolete && $metadata != null && $metadata['viewLink'] != '' && $actions[$service.'-link']['allowed'] ) {
				$actions[$service.'-link']['visible'] = true;
				$actions[$service.'-link']['href'] = $metadata['viewLink'];
				$actions[$service.'-link']['color'] = 'blueviolet';
			}
		}
		/**
...
function wfu_init_uploadedfiles_properties() {
	...
	/** This filter is documented in lib/wfu_admin_transfers.php */
	$services = apply_filters("_wfu_filetransfer_services", array());
	foreach ( $services as $service ) {
		$props[$service] = array(
			"icon"		=> "wfu-dashicons-".$service,
			"title"		=> "File has been transferred to ".wfu_service_get_name($service),
			"visible"	=> false,
			"remarks"	=> "",
			"code"		=> wfu_create_random_string(6)
		);
...
function wfu_init_uploadedfiles_actions() {
	...
	$services = apply_filters("_wfu_filetransfer_services", array());
	foreach ( $services as $service ) {
		$def_actions[$service."-link"] = array(
			"icon"		=> "wfu-dashicons-".$service."-link",
			"title"		=> "Open remote ".wfu_service_get_name($service)." file link",
			"allowed"	=> false,
			"visible"	=> false,
			"href"		=> "",
			"newtab"	=> true,
	...
	foreach ( $actions_raw as $ind => $action )
		...
		//adjust allowed property of cloud services remote links
		elseif ( $action == 'remotelinks' ) {
			foreach ( $services as $service ) {
				$actions[$service."-link"] = $def_actions[$service."-link"];
				$actions[$service."-link"]['allowed'] = true;
				unset($def_actions[$service."-link"]);
			}
		}
