	...
	if ( substr($path, 0, 7) == "sftp://" ) {
		...
		if ( !$ftpinfo["error"] ) {
			...
			if ( $pos1 ) {
				...
				if ( ( WFU_VAR("WFU_SFTP_USE_PHPSECLIB") == "true" || !function_exists("ssh2_connect") ) ) {
					include_once ABSWPFILEUPLOAD_DIR . WFU_AUTOLOADER_PHP50600;
					$sftp = new phpseclib\Net\SFTP($data["ftpdomain"], $ftp_port);
					if ( $sftp->login($data["username"], $data["password"]) ) {
						$result = $sftp->is_dir($path);
					}
				}
				else
				{
...
function wfu_upload_file($source, $target, $method, $ftpdata, $passive, $fileperms) {
	...
	elseif ( substr($method, 0, 7) == "chunked" ) {
		$is_first = ( substr($method, 7, 1) == "+" );
		$chunk_data = explode('|', substr($method, 8));
//		$chunk_num = substr($method, 8);
		$chunk_num = $chunk_data[0];
		$target .= $chunk_data[1];
		$sou = fopen($source, "rb");
		$pos = WFU_VAR("WFU_CHUNK_SIZE") * $chunk_num;
		$end = $pos + WFU_VAR("WFU_CHUNK_SIZE");
		$piece = wfu_create_string(WFU_VAR("WFU_CHUNK_SIZE"));
		if ( $sou ) {
			$no_error = true;
			//get file size, set zero if this is the first chunk
			$filesize = ( $is_first ? 0 : ( file_exists($target) ? wfu_bigfilesize($target) : 0 ) );
			//check if file must be enlarged before adding the chunk, which can happen if chunks are not coming with the correct order
			if ( $filesize < $pos ) {
				$more_size = $pos - $filesize;
				$reps = $more_size / WFU_VAR("WFU_CHUNK_SIZE");
				$rem = $more_size - WFU_VAR("WFU_CHUNK_SIZE") * $reps;
				$openmode = ( $is_first ? "wb" : "ab" );
				$tar = fopen($target, $openmode);
				if ( $tar ) {
					for ( $i = 0; $i < $reps; $i++ )
						if ( ($no_error = fwrite($tar, $piece)) === false ) break;
						else $filesize += $no_error;
					if ( $no_error !== false )
						if ( ($no_error = fwrite($tar, substr($piece, 0, $rem))) !== false ) $filesize += $no_error;
					fclose($tar);
				}
				else $no_error = false;
			}
			if ( $no_error !== false ) {
				$openmode = ( $is_first ? "wb" : ( $filesize > $pos ? "r+b" : "ab" ) );
				$tar = fopen($target, $openmode);
				if ( $tar ) {
					wfu_fseek($tar, $pos);
					$buff = fread($sou, WFU_VAR("WFU_CHUNK_SIZE"));
					$no_error = fwrite($tar, $buff);
				}
				else $no_error = false;
				$ret_array["uploaded"] = ( $no_error !== false ? true : false );
				if ( !$ret_array["uploaded"] ) $ret_message = sprintf(WFU_ERROR_ADMIN_CHUNKWRITEFAILED, $chunk_num);
			}
			else $ret_message = sprintf(WFU_ERROR_ADMIN_CHUNKENLARGEFAILED, $chunk_num);
		}
		else $ret_message = sprintf(WFU_ERROR_ADMIN_CHUNKHANDLEFAILED, $chunk_num);
		fclose($sou);
		fclose($tar);
	}
...
function wfu_upload_file_sftp($ftp_host, $ftp_port, $ftp_username, $ftp_password, $source, $target, $fileperms) {
	...
	if ( ( WFU_VAR("WFU_SFTP_USE_PHPSECLIB") == "true" || !function_exists("ssh2_connect") ) ) {
		...
		if ( !$sftp->login($ftp_username, $ftp_password) ) $ret_message = WFU_ERROR_ADMIN_FTPLOGIN_FAIL;
		else {
			$contents = @file_get_contents($source);
			if ( $contents === false ) $ret_message = WFU_ERROR_ADMIN_FTPSOURCE_FAIL;
			else {
				if ( !$sftp->put($target, $contents) ) $ret_message = WFU_ERROR_ADMIN_FTPTRANSFER_FAIL;
				else {
					$fileperms = trim($fileperms);
					if ( strlen($fileperms) == 4 && sprintf("%04o", octdec($fileperms)) == $fileperms ) {
						$fileperms = octdec($fileperms);
						$fileperms = (int)$fileperms;
						$sftp->chmod($fileperms, $target);
					}
				}
			}
		}
...
function wfu_stat_sftp($filepath) {
	...
	if ( ( WFU_VAR("WFU_SFTP_USE_PHPSECLIB") == "true" || !function_exists("ssh2_connect") ) ) {
		include_once ABSWPFILEUPLOAD_DIR . WFU_AUTOLOADER_PHP50600;
		$sftp = new phpseclib\Net\SFTP($data["ftpdomain"], $data["port"]);
		if ( $sftp->login($data["username"], $data["password"]) ) $ret = $sftp->stat($data["filepath"]);
	}
	else
	{
		$conn = @ssh2_connect($data["ftpdomain"], $data["port"]);
...
function wfu_file_get_contents_sftp($filepath) {
	...
	if ( ( WFU_VAR("WFU_SFTP_USE_PHPSECLIB") == "true" || !function_exists("ssh2_connect") ) ) {
		include_once ABSWPFILEUPLOAD_DIR . WFU_AUTOLOADER_PHP50600;
		$sftp = new phpseclib\Net\SFTP($data["ftpdomain"], $data["port"]);
		if ( $sftp->login($data["username"], $data["password"]) ) $ret = $sftp->get($data["filepath"]);
	}
	else
	{
		$conn = @ssh2_connect($data["ftpdomain"], $data["port"]);
